...
class UserArguments(object):
    ...
    def __init__(self, arg_dict: dict):
        ...
        self._merged_sdf: Optional[Path] = None
    ...
    def get_tc_threshold(self, fp_name) -> float:
        if self.dict[TANIMOTO_COEF_THRESHOLD] is config.DEFAULT_TC:
            ...
            return self.dict[TANIMOTO_COEF_THRESHOLD][
                self.dict[FINGERPRINT].index(fp_name)
            ]
    ...
    def sdf_path(self):
        if self._merged_sdf is None:
            if self.dict[SMILES]:  # SMILES input
                self.convert_smiles_to_sdf()
            elif len(self.dict[SDFile]) > 1:
                self.merge_sdf()
            else:
    ...
    def are_ok(self) -> bool:
        ...
        # Validate input format - either SDF files or SMILES strings, not both
        if not self.dict[SDFile] and not self.dict[SMILES]:
            are_they = False
            self.errormsg += (
                "Either SDF files or SMILES strings must be provided as input.\n"
            )

        if self.dict[SDFile] and self.dict[SMILES]:
            are_they = False
            self.errormsg += "Cannot use both SDF files and SMILES strings as input simultaneously.\n"
        ...
        # Validate SDF files if provided
        if self.dict[SDFile]:
            for sdf in self.dict[SDFile]:
                sdf_path = Path(sdf)
                if not sdf_path.is_file() or sdf_path.suffix != ".sdf":
                    are_they = False
                    self.errormsg += f"File {sdf} not found or not an SDF file.\n"

        # Validate SMILES strings if provided
        if self.dict[SMILES]:
            for smiles_entry in self.dict[SMILES]:
                if not smiles_entry.strip():
                    are_they = False
                    self.errormsg += "Empty SMILES string provided.\n"
        ...
        if self.dict[TANIMOTO_COEF_THRESHOLD] is not config.DEFAULT_TC:
            for tc in self.dict[TANIMOTO_COEF_THRESHOLD]:
                if tc < 0 or tc > 1:
                    are_they = False
                    self.errormsg += (
                        "Tanimoto coefficient value must be between 0 and 1.\n"
                    )

            if len(self.dict[TANIMOTO_COEF_THRESHOLD]) != len(self.dict[FINGERPRINT]):
                are_they = False
                self.errormsg += "Fingerprint number and Tanimoto threshold mismatch.\n"
        ...
        if self.dict[OUTPUT] is not config.DEFAULT_OUTPUT:
            if not Path(self.dict[OUTPUT]).parent.is_dir():
                are_they = False
                self.errormsg += (
                    f"Directory {Path(self.dict[OUTPUT]).parent} not found.\n"
                )
        ...
        if self.check_input_files() is False:
            are_they = False
    ...
    def check_input_files(self) -> bool:
        """
        Check if the input files (SDF) or SMILES strings have uniquely named molecules.
        """
        error = False
        msg = ""
            ...
            for file in self.dict[SDFile]:
                ...
                for mol_sdf_str in [m for m in mols_sdf_str if m != ""]:
                    if (
                        ...
                        ):  # Check if the name already appeared before
                            error = True
                            msg += texts.sdf_mol_name_duplicate_error.format(
                                file, mol_name
                            )
                        else:
                            mol_name_list.append(mol_name)

        elif self.dict[SMILES]:
            # Check SMILES strings
            for smiles_entry in self.dict[SMILES]:
                # Parse name:SMILES format
                if ":" in smiles_entry:
                    name, smiles = smiles_entry.split(":", 1)
                else:
                    name = smiles_entry  # Use SMILES as name if no name provided

                if name in mol_name_list:
                    error = True
                    msg += f"Duplicate molecule name found in SMILES input: {name}\n"
                else:
                    mol_name_list.append(name)

        self.number_query_molecule_found = len(mol_name_list)
        self.verbose_msg = (
            texts.molecules_found.format(self.number_query_molecule_found) + "\n"
        )
        self.errormsg += msg
        return not error

    def merge_sdf(self) -> None:
        ...
        for sdf in self.dict[SDFile]:
            ...
            mol_list.extend(
                [
                    m
                    for m in Path(sdf)
                    .read_text(encoding=config.ENCODING)
                    .split(sd_mol_delimiter)
                    if len(m) > 0
                ]
            )
        ...
        self._merged_sdf.write_text(
            sd_mol_delimiter.join(mol_list), encoding=config.ENCODING
        )

    def convert_smiles_to_sdf(self) -> None:
        """Convert SMILES strings to SDF format using RDKit."""
        try:
            from rdkit import Chem
            from rdkit.Chem import AllChem
        except ImportError:
            self.errormsg += "RDKit is required for SMILES input. Please install RDKit: pip install rdkit\n"
            raise RuntimeError(self.errormsg)
        ...
        for smiles_entry in self.dict[SMILES]:
            ...
            if ":" in smiles_entry:
                ...
                # If no name provided, use SMILES as name
                name = smiles_entry
                smiles = smiles_entry
            ...
            if mol is None:
                self.errormsg += f"Invalid SMILES string: {smiles}\n"
                continue
            ...
            # Convert to SDF block
            sdf_block = Chem.MolToMolBlock(mol)
            sdf_molecules.append(sdf_block)

        if not sdf_molecules:
            self.errormsg += "No valid SMILES strings provided.\n"
            raise RuntimeError(self.errormsg)

        self._merged_sdf = Path(config.DEFAULT_SMILES_SDF_NAME)
        self._merged_sdf.write_text(
            sd_mol_delimiter.join(sdf_molecules), encoding=config.ENCODING
        )
...
# Also used by the program to access arguments value as dictionary key
SDFile = "SDFile"
SMILES = "smiles"
DATABASE = "db"
FINGERPRINT = "fp"
...
def _get_argparse_setup() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=texts.abstract, formatter_class=argparse.RawTextHelpFormatter
    )

    # Add positional SDF argument
    parser.add_argument(
        SDFile,
        metavar="SDF",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[],
        help="SMILES strings as input. Provide as: name1:SMILES1 name2:SMILES2 ...",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=[config.DEFAULT_FP],
        help=texts.help_fp,
    )
    parser.add_argument(
        f"-{TANIMOTO_COEF_THRESHOLD}",
        dest=TANIMOTO_COEF_THRESHOLD,
        type=float,
        nargs="+",
        default=config.DEFAULT_TC,
        help=texts.help_tc,
    )
    parser.add_argument(
        f"-{ZSCORE_THRESHOLD}",
        ...
        type=float,
        default=config.DEFAULT_SD,
        help=texts.help_sd,
    )
    parser.add_argument(
        f"-{REPORTED_TARGET_NUMBER}",
        dest=REPORTED_TARGET_NUMBER,
        type=int,
        default=config.DEFAULT_NBT,
        help=texts.help_nbt,
    )
    parser.add_argument(
        f"-{OUTPUT}",
    ...
    parser.add_argument(
        ...
        dest=NUM_CORE,
        type=int,
        default=config.DEFAULT_NUM_CORE,
        help=texts.help_cpu,
    )
    parser.add_argument(
        f"-{FILTER_BEST_POSE_PER_TARGET}",
        dest=FILTER_BEST_POSE_PER_TARGET,
        action="store_true",
        help=texts.help_bppt,
    )
    parser.add_argument(
        f"-{NO_INFO}", dest=NO_INFO, action="store_true", help=texts.help_noinfo
    )
    return parser
